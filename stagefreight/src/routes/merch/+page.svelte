  ...
  <div class="relative container mx-auto px-4 py-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
      {#each products as product}
        <div class="bg-gray-900 p-6 rounded-lg shadow-lg text-center cursor-pointer hover:scale-105 hover:border hover:border-red-500 transition" on:click={() => { showModal = true; selectedProduct = product; }}>
          <img src={product.image} alt={product.name} class="w-full h-auto mb-4 rounded-md" />
          <h3 class="text-xl font-semibold text-red-500">{product.name}</h3>
...
<div class="fixed top-40 right-4 w-96 max-h-[75vh] bg-gray-900 text-white p-6 shadow-xl rounded-lg overflow-y-auto transform transition-transform">
  ...
  {#if $cart.length > 0}
    ...
    <div class="mt-6">
      ...
      <button
        on:click={checkout}
        class="w-full mt-6 px-6 py-3 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">
        Checkout
      </button>
    </div>
  {:else}
...
{#if showModal}
  <div class="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50">
    <div class="bg-gray-900 text-white p-8 rounded-lg w-96 shadow-lg">
      <h3 class="text-3xl font-semibold text-red-500 mb-4">{selectedProduct.name}</h3>
      ...
      <div class="mt-6 flex justify-between space-x-4">
        <button on:click={() => { addToCart(selectedProduct, selectedSize); showModal = false; }} class="flex-1 px-6 py-3 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">Add to Cart</button>
